...
function get_type_by_id($id){
    $type =  M('Type')->where(array('id'=>$id))->select();
    if($type){
        return $type[0]['name'];
    }
    return '未知';
...
function get_groups(){
    $groups =  get_catalog('/item-groups');
//    $groupsMap = array();
//    $enabled_groups = M('Type')->field('type_id')->select();
//
//    if($groups != null && isset($groups['results']) && $groups['results'] != null){
//        foreach($groups['results'] as $group){
//            $groupsMap[$group['groupId']] = $group;
//        }
//
//        if($enabled_groups != null && is_array($enabled_groups)){
//            foreach($enabled_groups as $enabled){
//                if(array_key_exists($enabled['type_id'],$groupsMap)){
//                    unset($groupsMap[$enabled['type_id']]);
//                }
//            }
//        }
//    }
