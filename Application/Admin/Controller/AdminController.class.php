...
class AdminController extends Controller {
    ...
    protected function getMenu(){
        ...
        //生成每个分类的url
        foreach ($cate as $key=>&$value){
            $value['url']   =   self::buildUrl($value['id']);//'Article/index?cate_id='.$value['id'];
            ...
            if(!empty($value['_child'])){
                ...
                foreach ($value['_child'] as $ka=>&$va){
                    $va['url']      =   self::buildUrl($va['id']);//'Article/index?cate_id='.$va['id'];
                    if(!empty($va['_child'])){
                        foreach ($va['_child'] as $k=>&$v){
                            $v['url']   =   self::buildUrl($v['id']);//'Article/index?cate_id='.$v['id'];
                            $v['pid']   =   $va['id'];
    ...
    private function buildUrl($cate_id){
        $cate =  get_category($cate_id);
        $map = array('id' => $cate['model']);
        $model = M('Model')->where($map)->select();
        if($model[0]['extend'] == 1){
            return 'Article/index?cate_id='.$cate_id;
        } else {
            return $model[0]['name'].'/index?cate_id='.$cate_id;
        }
    }
