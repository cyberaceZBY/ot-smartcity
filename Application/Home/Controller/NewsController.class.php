...
class NewsController extends HomeController {
    ...
    public function detail($id = 0, $p = 1){
        ...
        if(!empty($info['template'])){//已定制模板
            ...
            $tmpl = 'News/detail';//get_document_model($info['model_id'],'name') .'/detail';
        }
        ...
        if(!empty($info['relation'])){
            $temp = json_decode($info['relation']);
            foreach($temp as $key => $r){
                $relation[$key]['title']=$r->title;
                $relation[$key]['link']=$r->link;
            }
        }
...
//        $reports    = M('document')->order('level ASC,id DESC')->select();
        ...
        foreach($newsList as $news) {
            foreach($newsContentList as $newsContent){
                if ($news['id'] == $newsContent['id']) {
                    if ($newsContent['index_type'] == $typeId) {
                        $finalNews[] = $news;
                    }
                }
            }
        ...
        /* 模板赋值并渲染模板 */
        $this->assign('news', $finalNews);
//        $this->assign('types', $finalNewsType);
        $this->assign('contentId', $typeId);
        $this->display();
    }
